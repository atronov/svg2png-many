...
function svg2PngDirs(srcDir, dstDir) {

    var processFile = file => {
        var srcPath = file;
        var fileName = path.parse(file).name;
        var dstPath = path.join(dstDir, fileName + ".png");
        return convert(instance, srcPath, dstPath);
    };

    var filesToConvert = fs.readdirSync(srcDir)
        .filter(name => SVG_REGEX.test(name.trim()))
    ...
    return phantom.create()
        .then(instance => {
            ...
            return convertMany(instance, filesToConvert, dstDir);
        })
